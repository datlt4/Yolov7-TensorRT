...
std::vector<bbox_t> Yolov7TRT::EngineInference(cv::Mat& image)
{
    ...
    std::sort(boxes.begin(), boxes.end(), [](bbox_t A, bbox_t B) {
...
std::vector<float> Yolov7TRT::prepareImage(cv::Mat& img)
{
    ...
    std::vector<cv::Mat> split_img = { cv::Mat(IMAGE_HEIGHT, IMAGE_WIDTH, CV_32FC1, data + channelLength * 2), cv::Mat(IMAGE_HEIGHT, IMAGE_WIDTH, CV_32FC1, data + channelLength), cv::Mat(IMAGE_HEIGHT, IMAGE_WIDTH, CV_32FC1, data) };
...
{
    sort(detections.begin(), detections.end(), [=](const bbox_t& left, const bbox_t& right) { return left.prob > right.prob; });
    ...
    detections.erase(std::remove_if(detections.begin(), detections.end(), [](const bbox_t& det) { return det.prob == 0; }), detections.end());
}
